...
def test_build_nn(build_nn):
    with tf.Graph().as_default():
        ...
        #assert final_state.get_shape().as_list() == [test_rnn_layer_size, 2, None, test_rnn_size], \
         #   'Final state wrong shape.  Found shape {}'.format(final_state.get_shape())
        """
        Commented above to get rid off BS message because of higher version (1.4) of TF,
        I had hard time setting up this thing and I would not downgrade it just to pass a test case.
        As I have one or two side projects running too, ANd I would not like to break it
        """
